<template>
  <div class="container">
      ...
      <div class="charger-container">
        ...
        <div class="status-summary">
          ...
          <div class="status-item">
            ...
            <div class="status-value">78.5%</div>
          </div>
        </div>
        ...
        <div class="charger-grid">
          <div 
            ...
            :key="index" 
            class="charger-item"
            @mouseover="showTooltip(charger, $event)"
            ...
            <div class="charger-content">
              ...
              <div class="charger-info">
                ...
                <div class="info-item">
                  ...
                  <span class="info-value">{{ (charger.consumed * 0.06).toFixed(2) }} 元</span>
                </div>
              </div>
            </div>
          </div>
    ...
    <!-- 充电桩详细信息弹窗 -->
    <div 
      v-if="showChargerTooltip && currentCharger" 
    ...
    >
      ...
      <div class="tooltip-content">
        <div class="tooltip-row">
          <span class="tooltip-label">电流:</span>
          <span class="tooltip-value">{{ currentCharger.current }} A</span>
        </div>
        <div class="tooltip-row">
          <span class="tooltip-label">电压:</span>
          <span class="tooltip-value">{{ currentCharger.voltage }} V</span>
        </div>
        <div class="tooltip-row">
          <span class="tooltip-label">温度:</span>
          <span class="tooltip-value">{{ currentCharger.temperature }} ℃</span>
        </div>
        <div class="tooltip-row">
          <span class="tooltip-label">当前功率:</span>
          <span class="tooltip-value">{{ currentCharger.power }} kW</span>
        </div>
        <div class="tooltip-row">
          <span class="tooltip-label">已充电量:</span>
          <span class="tooltip-value">{{ currentCharger.consumed }} kWh</span>
        </div>
        <div class="tooltip-row">
          <span class="tooltip-label">使用时长:</span>
          <span class="tooltip-value">{{ currentCharger.duration }} 分钟</span>
        </div>
      </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    // 工具提示相关状态
    const showChargerTooltip = ref(false);
    const currentCharger = ref<any>(null);
    const tooltipPosition = ref({ x: 0, y: 0 });
    
    // 显示工具提示
    const showTooltip = (charger: any, event: MouseEvent) => {
      currentCharger.value = charger;
      showChargerTooltip.value = true;
    ...
    // 隐藏工具提示
    const hideTooltip = () => {
      showChargerTooltip.value = false;
    ...
    const fetchChargerData = async () => {
      try {
        const response = await api.get('/api/electric-info');
        const data = response.data;
        
        // 转换数据格式并计算功率
        chargers.value = data.map((item: any) => {
          // 计算功率 (功率 = 电流 * 电压 / 1000)
          ...
          const duration = Math.floor(Math.random() * 60 + 10); // 随机使用时长
        ...
        // 使用模拟数据作为后备
        chargers.value = [
          { id: '1', status: 'normal', power: 7.2, consumed: 24.8, duration: 45, current: '40', voltage: '500', temperature: '43' },
          { id: '2', status: 'normal', power: 6.8, consumed: 18.3, duration: 32, current: '41.6', voltage: '480.5', temperature: '45.2' },
          { id: '3', status: 'abnormal', power: 0.0, consumed: 0.0, duration: 0, current: '79.2', voltage: '493.3', temperature: '99.4' },
          { id: '4', status: 'normal', power: 7.5, consumed: 32.1, duration: 58, current: '60.2', voltage: '470.9', temperature: '52.4' },
          { id: '5', status: 'normal', power: 6.5, consumed: 15.7, duration: 30, current: '50', voltage: '400', temperature: '48.7' },
          { id: '6', status: 'normal', power: 7.0, consumed: 28.9, duration: 55, current: '30', voltage: '666.7', temperature: '39.8' },
          { id: '7', status: 'abnormal', power: 0.0, consumed: 0.0, duration: 0, current: '93.2', voltage: '5.8', temperature: '103.4' },
          { id: '8', status: 'normal', power: 6.9, consumed: 20.4, duration: 38, current: '35.3', voltage: '571.4', temperature: '41.2' },
          { id: '9', status: 'normal', power: 7.3, consumed: 26.7, duration: 50, current: '69.2', voltage: '450.3', temperature: '37.6' },
          { id: '10', status: 'normal', power: 6.7, consumed: 17.2, duration: 35, current: '28', voltage: '444.8', temperature: '55.4' }
        ];
      }
    };
...
.charger-tooltip {
  ...
  z-index: 1000;
  min-width: 280px;
  background: linear-gradient(145deg, rgba(16, 36, 73, 0.95), rgba(8, 18, 37, 0.98));
  border: 1px solid rgba(0, 167, 255, 0.5);
  border-radius: 8px;
  padding: 15px;
  box-shadow: 
    0 0 20px rgba(0, 167, 255, 0.3),
...
.tooltip-header {
  ...
  margin-bottom: 12px;
  padding-bottom: 10px;
  border-bottom: 1px solid rgba(125, 157, 204, 0.3);
}
...
.tooltip-footer {
  margin-top: 15px;
  padding-top: 10px;
  border-top: 1px solid rgba(125, 157, 204, 0.2);
  color: #7d9dcc;
  font-size: 12px;
  text-align: right;
}
